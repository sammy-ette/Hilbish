...
:::warning
It is not recommended to set Hilbish as your login shell. That
...
Some shells (like zsh) have an rc file, like `.zlogin`, which is run when the shell session
...
As an example, it will usually result in `/usr/share/hilbish/.hilbishrc.lua` on Linux.
