...
- [`commander.get(name) -> fun(args:table,sinks:table<string,Sink>):number?`](#get): Returns the registered command function for `name`, or `nil` if it does not exist.
...
:::signature
```lua
commander.get(name) -> fun(args:table,sinks:table<string,Sink>):number?
```
:::

Returns the registered command function for `name`, or `nil` if it does not exist.
...
:::returns
`fun(args:table,sinks:table<string,Sink>):number?` [Optional]{.optional}
