...
function hilbish.completions.handler(line, pos)
	...
	if #fields == 1 then
		local comps, pfx = hilbish.completions.bins(query, ctx, fields, hilbish.opts.insensitive)
		local compGroup = {
			items = comps,
			type = 'grid'
		}
		...
		local comps, pfx = hilbish.completions.files(query, ctx, fields, hilbish.opts.insensitive)
		local compGroup = {
			items = comps,
			type = 'grid'
		}
